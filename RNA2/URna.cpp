	...
	emqv_anterior =       erro_medio_quadratico_validacao;
		...
		//FmRna->Memo1->Lines->Add("padrões de treinamento");
		 std::string file_name_saidas = "../Saidas/" + std::to_string(contador) + ".txt";
				...
				if (!fp_saidas) {
					std::cerr << "Erro ao criar arquivo de saídas: " << file_name_saidas << std::endl;
					break;
				}
...
//-----------------------------------------------------------------------------//
	...
	//-----------------------------------------------------------------------------//
		...
		if (saidasFile.is_open()) {
			...
			std::cerr << "Erro ao abrir arquivo de saídas: " << file_name_saidas << std::endl;
		}
				...
				while (i <= bytesRead - 8)
				{
					if (buffer[i] == '#' && buffer[i + 1] == '$' && buffer[i + 2] == ':')
					{
						...
						if (buffer[i + 7] == checksum)
						{
							...
							{
								conta_pontos ++;
								// Se a posição do gráfico atingir 1000, reseta para 0
								if (conta_pontos >= 1000)
								{
									...
									for (size_t i = 0; i < dados_tempo_real.size(); i += 1000) {
										...
										// Garantir que temos 250 amostras à esquerda e à direita do pico
									...
									// Atualiza o gráfico na posição atual
...
void __fastcall TFmRna::ListComPorts() {
	...
	if (deviceInfoSet == INVALID_HANDLE_VALUE) {
		ShowMessage("Erro ao obter informações das portas COM");
		return;
	}
